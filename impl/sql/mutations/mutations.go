...
func (m *mutations) ReadRange(txn transaction.Txn, startSequence, endSequence uint64, count int32) (uint64, []*pb.SignedKV, error) {
...
// sequence number read.
func (m *mutations) ReadAll(txn transaction.Txn, startSequence uint64) (uint64, []*pb.SignedKV, error) {
...
func readRows(rows *sql.Rows) (uint64, []*pb.SignedKV, error) {
	results := make([]*pb.SignedKV, 0)
...
// sequence number.
func (m *mutations) Write(txn transaction.Txn, mutation *pb.SignedKV) (uint64, error) {
