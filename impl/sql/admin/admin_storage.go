...
func New(db *sql.DB) (admin.Storage, error) {
	s := &storage{
		db: db,
	}
	// Create tables.
...
func (s *storage) List(ctx context.Context, showDeleted bool) ([]*admin.Domain, error) {
	var query string
	if showDeleted {
...
func (s *storage) Read(ctx context.Context, ID string, showDeleted bool) (*admin.Domain, error) {
	var SQL string
	if showDeleted {
		SQL = readDeletedSQL
	} else {
