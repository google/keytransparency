...
var (
	filter_KeyTransparencyAdminService_BatchSetEntries_0 = &utilities.DoubleArray{Encoding: map[string]int{}, Base: []int(nil), Check: []int(nil)}
)

func request_KeyTransparencyAdminService_BatchSetEntries_0(ctx context.Context, marshaler runtime.Marshaler, client KeyTransparencyAdminServiceClient, req *http.Request, pathParams map[string]string) (proto.Message, runtime.ServerMetadata, error) {
	var protoReq keytransparency_v1_types.BatchSetEntriesRequest
	var metadata runtime.ServerMetadata

	if err := runtime.PopulateQueryParameters(&protoReq, req.URL.Query(), filter_KeyTransparencyAdminService_BatchSetEntries_0); err != nil {
		return nil, metadata, grpc.Errorf(codes.InvalidArgument, "%v", err)
	}

	msg, err := client.BatchSetEntries(ctx, &protoReq, grpc.Header(&metadata.HeaderMD), grpc.Trailer(&metadata.TrailerMD))
	return msg, metadata, err
...
func RegisterKeyTransparencyAdminServiceHandler(ctx context.Context, mux *runtime.ServeMux, conn *grpc.ClientConn) error {
	...
	mux.Handle("POST", pattern_KeyTransparencyAdminService_BatchSetEntries_0, func(w http.ResponseWriter, req *http.Request, pathParams map[string]string) {
		ctx, cancel := context.WithCancel(ctx)
		defer cancel()
		if cn, ok := w.(http.CloseNotifier); ok {
			go func(done <-chan struct{}, closed <-chan bool) {
		...
		resp, md, err := request_KeyTransparencyAdminService_BatchSetEntries_0(rctx, inboundMarshaler, client, req, pathParams)
		ctx = runtime.NewServerMetadataContext(ctx, md)
		if err != nil {
			runtime.HTTPError(ctx, outboundMarshaler, w, req, err)
			return
		...
		forward_KeyTransparencyAdminService_BatchSetEntries_0(ctx, outboundMarshaler, w, req, resp, mux.GetForwardResponseOptions()...)
...
var (
	pattern_KeyTransparencyAdminService_BatchSetEntries_0 = runtime.MustPattern(runtime.NewPattern(1, []int{2, 0, 2, 1}, []string{"v1", "users"}, "batch"))
)

var (
	forward_KeyTransparencyAdminService_BatchSetEntries_0 = runtime.ForwardResponseMessage
)
