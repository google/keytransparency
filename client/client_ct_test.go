...
// TestVerifySCT exercieses an immmediate verification via an inclusion proof as
...
func TestVerifySCT(t *testing.T) {
	...
	hs := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		switch {
		case r.URL.Path == "/ct/v1/get-sth":
			w.Write([]byte(tests[j].sth))
		case r.URL.Path == "/ct/v1/get-sth-consistency":
			w.Write([]byte(tests[j].consistency))
		case r.URL.Path == "/ct/v1/add-json":
			w.Write([]byte(tests[j].sct))
		case r.URL.Path == "/ct/v1/get-proof-by-hash":
			r.ParseForm()
			if got, want := r.Form["hash"][0], tests[j].hash; got != want {
				t.Errorf("Incorrect request hash:\n%v, wanted\n%v", got, want)
			}
			w.Write([]byte(tests[j].inclusion))
		default:
			t.Fatalf("Incorrect URL path: %s", r.URL.Path)
		}
	}))
...
// TestVerifySCTs ensures that cached SCTs are verified.
func TestVerifySCTs(t *testing.T) {
	...
	for i, tc := range tests {
		...
		if entries := l.VerifySCTs(); len(entries) != 0 {
			t.Errorf("%v: VerifySCTs(): %v", i, entries)
		}
	}
}
