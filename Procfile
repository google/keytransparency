...
web: ./key-transparency --port=$PORT --key=$KEY --cert=$CERT --domain=$DOMAIN --db=$DB --maplog=$CTLOG --etcd=$LISTEN --vrf=$VRF_PRIV -logtostderr=true 
sign: ./e2e-key-signer --domain=$DOMAIN --db=$DB  --maplog=$CTLOG --etcd=$LISTEN --period=$SIGN_PERIOD_SEC --key=$SIGN_KEY
