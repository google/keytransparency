...
const (
	...
	oneOffStr    = "2015-10-21T03:29:00+00:00"
...
func TestEpochCreation(t *testing.T) {
	clock := fakeClock{now: parseTime(t, fakeNow)}
	now := clock.Now()
	for _, tc := range []struct {
		...
		{1, now, parseTime(t, oneOffStr), minInMax, minDurationS, maxDurationH},
		{1, now, parseTime(t, sixOff), minInMax / 2, minDurationS, maxDurationH},
		{1, now, parseTime(t, threeOff), minInMax, minDurationS, maxDurationH},
		// one forced epoch after 1 hour and one after 7 hours:
		{2, now, parseTime(t, threeOff), minInMax + int(minInMax/2), minDurationS, maxDurationH},
		// One forced epoch "after 4 = -2+6 hours" catching up and 3 forced epochs
		// "after 10, 16, and 22 hours"
		{4, now, parseTime(t, twoOff), minInMax * 4, minDurationS, maxDurationH},
	} {
...
func parseTime(t *testing.T, ts string) time.Time {
	...
	if err != nil {
		t.Fatalf("parseTime(_, %v) expects time.RFC3339 formatted time strings as argument", ts)
	}
	return ti
}
