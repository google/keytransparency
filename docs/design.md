...
Key Transparency provides methods for auditing both current and past values that the server returned for an account. This audit ability is composed of the following cryptographically verifiable primitives:

*   Everyone is querying the same data structure, as verified by a gossip protocol. 
*   Each user has a single entry in the data structure, as verified by a unique index. 
*   As user data changes and therefore the whole data structure, each change is stored in an append-only data structure.

Together, these properties mean that even a malicious server cannot insert / remove / tamper with a user’s keys without leaving a permanent trail of evidence that a user can automatically discover.
### Threat Model 
...
If an attacker can modify a client of Key Transparency without being detected,
the adversary could modify the client to forgo performing the proper account
verification steps and hide a man-in-the-middle attack from the user.  To
detect the man-in-the-middle attack, a user just needs a clean verification
client which may be available on another device, compiled from source on
github.com, or navigated to on a 3rd party verification website.  
...
A key transparency server could update a user’s keys in a transparent, user
visible fashion that is analogous to an account-reset - but then deny a user
the ability to subsequently remove or update unwanted public keys under their
account. This is permitted under this threat model, and may be employed in
enterprise or compelled assistance environments. In either case, the presence
of a 3rd party account access is visible to the user and the public keys are in
the public record. 
...
addresses and keys, along with what app a key is for. This mapping, however, is
protected by cryptographic primitives which reduce the public map to a map of
opaque keys to opaque values. An observer of this map can see an upper bound on
how many entries (users) there are in the map, and how quickly the map is
changing and growing (new users / user activity).  
...
*   [gRPC API](docs/api_gen.md)
*   [HTTP API](docs/http_apis.md)
...
For PGP keys, however, the server will enforce a limit of 1 PGP keyring per
account. 
...
The keys that a recipient sees for him/herself are the same keys a potential
...
row. And again with the second row and third until there's only one value at
the top.  We call this the STR, and we use it for the hash we previously talked
about.
...
Now I need to prove to you that a given key is part of a root hash. To do that,
I need you to be able to calculate the root hash yourself. I need to give you
the key (k) you asked for and each neighboring node (n) up the tree so you can
compute the path to the root (x) yourself like this:
...
To prove that there's only one key for you (alice@gmail.com), the server takes
your email address and hashes it to produce a string of bits.
`H(alice@gmail.com) = 010` (actually a VUF described in
the next section) for example. If we think of the Merkle Tree as a binary
search tree, with each bit in the hash indicating "left", or "right", through a
tree that's as deep as the hash output is long, we have a unique location in
the tree for you. No more surreptitious shadow keys.
...
### [Verification by Key Owners](docs/verification.md)

The only person that really knows if a key belongs to them is the user
themselves. 

User clients will query the server for their own values regularly, looking for
unknown keys.  All the user’s clients should be notified when new keys are
detected. New keys may or may not actually belong to the user. 
...
versions of the sparse tree head for the same epoch. 

Monitors also verify any public properties of leaf nodes such as signatures and ordering. 
...
logarithmic in efficiency. The CT team is also reengineering the CT servers to
support 10^12 entries, which should be more than enough.

## What Happens When...

#### A user loses all their keys?

The user does an account reset
...
Clients will either keep using old key values until one hash chain gets longer than the other. 

Clients should also report the equivocation to a monitoring system. 

#### How can a server be revoked? 

TBD pending [Key Transparency Discovery](docs/discovery.md)
